...
function Circle(x, y, radius, color, randomColor) {
  this.x = x;
  this.y = y;
  this.radius = radius;
  this.color = color;
  ...
  this.draw = function () {
    ...
    ctx.arc(this.x, this.y, radius, 0, (Math.PI / 180) * 180 * 2, false);
    ctx.fill();
  };
}
...
// const circleThree = new Circle(350, 50, 40, "yellow");
...
for (let i = 0; i < 20; i++) {
  ...
  const y = Math.floor(Math.random() * (canvas.width - radius * 2)) + radius;
